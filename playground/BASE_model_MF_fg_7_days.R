...
for (f in train_files){
  ...
  data_train_bin <-data_train_bin %>%
    mutate(type = if_else(eventtime > 7, "Censored", type),
  ...
  # build model
  fg_model <- riskRegression::FGR(update.formula(prodlim::Hist(eventtime,type)~., form), data_train_bin, cause = 1)
  ...
  # data_test_bin$pred_test[complete.cases(data_test_bin[,predictors_col])] <- predict(fg_model, newdata=data_test_bin[complete.cases(data_test_bin[,predictors_col]),], times = 7)
  ...
  data_test_bin$y_true_cat <-  if_else(data_test_bin$type == 1, "CLABSI", 
                                       if_else(data_test_bin$type == 2, "Death",
                                               if_else(data_test_bin$type == 3, "Discharge", "Censored")))       
...
##############################################################################################################################
load("C:/CLABSI/Garbage/clabsi_r/playground/2_Model_Structure_for_Baseline_and_Dynamic_Model/predictions/BASE/preds_BASE_MF_FG_7_DAYS_fg_7 days_MF")
load("C:/CLABSI/Garbage/clabsi_r/playground/2_Model_Structure_for_Baseline_and_Dynamic_Model/performances/BASE/results_BASE_MF_FG_7_DAYS_fg_7 days_MF")
