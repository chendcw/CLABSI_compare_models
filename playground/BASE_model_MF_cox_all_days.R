...
for (f in train_files){
  ...
  data_train_bin <-data_train_bin %>% 
    mutate(type = if_else(type == "CLABSI", 1, 0))
    # mutate(CLABSI_7d = if_else(type == "CLABSI" & eventtime <= 7, 1, 0)) 
  ...
  # data_test_bin$test_preds <- 1 - pec::predictSurvProb(cox_model, newdata = data_test_bin, time = 7)
  # or use riskRegression package and both give the same results
  ...
  data_test_bin$y_true_cat <- if_else(data_test_bin$type == 1 & data_test_bin$eventtime <= 7, "CLABSI", "Censored")
  data_test_bin$y_true_time <-  if_else(data_test_bin$type == 1 & data_test_bin$eventtime <= 7, data_test_bin$eventtime, 7)    
  
  data_test_bin$y_test <- if_else(data_test_bin$type == 1 & data_test_bin$eventtime <= 7, 1, 0)
...
##############################################################################################################################
load("C:/CLABSI/Garbage/clabsi_r/playground/2_Model_Structure_for_Baseline_and_Dynamic_Model/predictions/BASE/preds_BASE_MF_COX_ALL_DAYS_cox_all days_MF")
load("C:/CLABSI/Garbage/clabsi_r/playground/2_Model_Structure_for_Baseline_and_Dynamic_Model/performances/BASE/results_BASE_MF_COX_ALL_DAYS_cox_all days_MF")
