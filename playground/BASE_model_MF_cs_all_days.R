...
for (f in train_files){
  ...
  coef_cs_model <- coef(cs_model$models$`Cause CLABSI`)
  ...
  data_test_bin$pred_test <- predict(cs_model, newdata = data_test_bin, times = 7, cause = "CLABSI")$absRisk
...
##############################################################################################################################
load("C:/CLABSI/Garbage/clabsi_r/playground/2_Model_Structure_for_Baseline_and_Dynamic_Model/predictions/BASE/preds_BASE_MF_CS_ALL_DAYS_cs_all days_MF")
load("C:/CLABSI/Garbage/clabsi_r/playground/2_Model_Structure_for_Baseline_and_Dynamic_Model/performances/BASE/results_BASE_MF_CS_ALL_DAYS_cs_all days_MF")
...
Calibrate_summary <- results  %>% 
  ...
  mutate(mean = round(mean(value, na.rm = TRUE), 3),
