...
for (f in train_files){
  ...
  # build model
  cs_model <- riskRegression::CSC(update.formula(Hist(eventtime,type)~., form), data_train_bin, method = "breslow")
  coef_cs_model <- coef(cs_model$models$`Cause 1`)
  ...
  data_test_bin$pred_test <- predict(cs_model, newdata = data_test_bin, times = 7, cause = 1)$absRisk
...
##############################################################################################################################
load("C:/CLABSI/Garbage/clabsi_r/playground/2_Model_Structure_for_Baseline_and_Dynamic_Model/predictions/BASE/preds_BASE_MF_CS_7_DAYS_cs_7 days_MF")
load("C:/CLABSI/Garbage/clabsi_r/playground/2_Model_Structure_for_Baseline_and_Dynamic_Model/performances/BASE/results_BASE_MF_CS_7_DAYS_cs_7 days_MF")
