...
model <- "BASE_MF_MULTINOMIAL_7_DAYS"
model_type <- "multi"
horizon <- "7 days"
imputation <- "MF"
...
for (f in train_files){
  ...
  data_train_bin <-data_train_bin %>% 
    # mutate(CLABSI = if_else(type == "CLABSI" & eventtime <= 7, "CLABSI", "no_CLABSI")) %>%
    # mutate(CLABSI = relevel(factor(CLABSI), ref = "no_CLABSI")) %>%
    mutate(type = if_else((type == "CLABSI"|type == "Death"|type == "Discharge") & eventtime > 7, "Censored", type)) 
  ...
  # build model
  multinomial_model <- nnet:::multinom(form, data_train_bin)
  coef_multi_model <- summary(multinomial_model)$coefficients[1,]
  ...
  data_test_bin$pred_test <- predict(multinomial_model, newdata = data_test_bin, type = "probs")[, 2]
...
##############################################################################################################################
load("C:/CLABSI/Garbage/clabsi_r/playground/2_Model_Structure_for_Baseline_and_Dynamic_Model/predictions/BASE/preds_BASE_MF_MULTINOMIAL_7_DAYS_multi_7 days_MF")
load("C:/CLABSI/Garbage/clabsi_r/playground/2_Model_Structure_for_Baseline_and_Dynamic_Model/performances/BASE/results_BASE_MF_MULTINOMIAL_7_DAYS_multi_7 days_MF")
