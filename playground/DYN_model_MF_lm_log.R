...
for (f in train_files){
  ...
  data_train_bin <- data_train_bin %>%
    mutate(type = ifelse(eventtime <= LM + 7 & type == "CLABSI", 1, 0),
  ...
  # simple
  # use LMsuperlog0
  LMsuperlog0 <- glm(form, data_train_bin, family = binomial())
  
  coef_log_model <- coef(LMsuperlog0)
  ...
  data_test_bin$pred_test <- predict(LMsuperlog0, newdata = data_test_bin, type = "response")
  
  data_test_bin$y_test <-  data_test_bin$type
  data_test_bin$y_true_cat <-  if_else(data_test_bin$type == 1, "CLABSI", "no_CLABSI")
