...
for (f in train_files){
  ...
  data_train_bin <-data_train_bin %>% 
    mutate(type = if_else(type == "CLABSI" & eventtime <= 7, 1, 0),
           eventtime = if_else(eventtime > 7, 7, eventtime)) 
  ...
  # build model
  cox_model <- survival::coxph(update.formula(survival::Surv(eventtime, type)~., form), data_train_bin, x = TRUE)
  coef_cox_model <- coef(cox_model)
  
  # risk = function(model, newdata, time) {
  #   as.numeric(1-summary(survival::survfit(model, newdata = newdata, se.fit = F, conf.int = F), times = time)$surv)
  # }
  # test_preds <- risk(cox_model, newdata = data_test_bin, time = 7)
  ...
  data_test_bin$pred_test <- riskRegression::predictRisk(cox_model, newdata = data_test_bin, times = 7)
  # this is consistent with the results calculated by survfit
  # surv_probs_Cox <- survfit(cox_model, newdata = data_test_bin)
  # data_test_bin$test_preds <- 1- matrix(summary(surv_probs_Cox, times = 7)$surv)
  ...
  data_test_bin$y_true_cat <- ifelse(data_test_bin$type == 1, "CLABSI", "Censored")
...
##############################################################################################################################
load("C:/CLABSI/Garbage/clabsi_r/playground/2_Model_Structure_for_Baseline_and_Dynamic_Model/predictions/BASE/preds_BASE_MF_COX_7_DAYS_cox_7 days_MF")
load("C:/CLABSI/Garbage/clabsi_r/playground/2_Model_Structure_for_Baseline_and_Dynamic_Model/performances/BASE/results_BASE_MF_COX_7_DAYS_cox_7 days_MF")
...
predictions_sub <- predictions %>% 
  mutate(y_test = ifelse(predictions$y_true_cat == "CLABSI", 1, 0)) %>%
...
cutpointr(data = predictions_sub, 
          x = preds, class = y_true_cat, subgroup = test_set,
          pos_class = "CLABSI", direction = ">=") %>% 
