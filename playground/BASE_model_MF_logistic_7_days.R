...
for (f in train_files){
  ...
  data_train_bin <-data_train_bin %>% 
    mutate(type = if_else(type == "CLABSI" & eventtime <= 7, "CLABSI", "no_CLABSI")) %>%
    mutate(type = relevel(factor(type), ref = "no_CLABSI")) 
  ...
  form <- as.formula(                      # Create formula
    paste(outcome_col, " ~ LAB_WBC_count_last_log + LAB_CRP_last_log + CARE_VS_systolic_BP_last_nodes +", paste0(predictors_col[!predictors_col %in% vars_not_in_model], collapse = " + ")))
  
  # build model
  logistic_model <- glm(form, data_train_bin, family = "binomial")
  coef_log_model <- coef(logistic_model)
  ...
  data_test_bin$pred_test <- predict(logistic_model, newdata = data_test_bin, type = "response")
  ...
  coefs <- coefs %>% 
    add_row(variable = names(coef_log_model),
            value = coef_log_model,
            train_set = str_replace(f, path_data_complete, ""),
            model = model,
            model_type = model_type,
            horizon = horizon,
...
##############################################################################################################################
load("C:/CLABSI/Garbage/clabsi_r/playground/2_Model_Structure_for_Baseline_and_Dynamic_Model/predictions/BASE/preds_BASE_MF_LOGISTIC_7_DAYS_bin_7 days_MF")
load("C:/CLABSI/Garbage/clabsi_r/playground/2_Model_Structure_for_Baseline_and_Dynamic_Model/performances/BASE/results_BASE_MF_LOGISTIC_7_DAYS_bin_7 days_MF")
